...
from control_model.ControlNet.ldm.util import default
from control_model.ControlNet.ldm.modules.diffusionmodules.util import make_ddim_sampling_parameters, make_ddim_timesteps, noise_like, extract_into_tensor
import pdb
...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
        ...
        for i, step in enumerate(iterator):
            ...
            outs = self.p_sample_ddim(img, cond, ts, index=index, use_original_steps=ddim_use_original_steps,
                                      ...
                                      unconditional_guidance_scale=unconditional_guidance_scale,
                                      unconditional_conditioning=unconditional_conditioning,
                                      dynamic_threshold=dynamic_threshold,inpaint=inpaint)
            img, pred_x0 = outs
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      ...
                      dynamic_threshold=None,inpaint=None):
...
class DDIMSampler_ReferenceOnly(object):
    ...
    def ddim_sampling(self, cond, shape,
        ...
        if 'feature_control' in cond and cond['feature_control'] is not None:
            fea_control = cond['feature_control']
            #for i in range(1000):
                #t = torch.full((b,), i, device=device, dtype=torch.long)
            #import pdb;pdb.set_trace()
            img = self.model.q_sample(x_start = fea_control, t = torch.tensor([999]).to(device))
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if 'more_appearance' in c and c['more_appearance'] is not None:
            cond_image_start = torch.cat(c['more_appearance'], 1)
            # cond_image_start = self.model.get_first_stage_encoding(self.model.encode_first_stage(cond_image_hint))
            m_reference_image_noisy = cond_image_start
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            ...
            if 'image_control' in unconditional_conditioning and unconditional_conditioning['image_control'] is not None:
                ...
                # pdb.set_trace()
                # yuming !
                model_uncond, model_t = self.model.apply_model(x_in, t_in, c_in, reference_image_noisy_in).chunk(2) # , reference_image_noisy
            else:
                x_in = x_In
                t_in = t
                c_in = c
                ...
                if 'more_appearance' in c and c['more_appearance'] is not None:
                    model_t = self.model.apply_model(x_in, t_in, c_in, reference_image_noisy_in, more_reference_image_noisy=m_reference_image_noisy)
                    #import pdb;pdb.set_trace()
                else:
                    model_t = self.model.apply_model(x_in, t_in, c_in, reference_image_noisy_in)
