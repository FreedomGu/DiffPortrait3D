...
from control_model.ControlNet.ldm.util import exists
from control_model.ControlNet.ldm.modules.motion_module import get_motion_module, VanillaTemporalModule
    ...
    def __init__(
        ...
        for level, mult in enumerate(channel_mult): 
            ...
            if level != len(channel_mult) - 1:
                ...
                if self.use_motion_module:
                    in_channel_mm = out_ch or ch
                    self.input_blocks_motion_module.append(
                        TimestepEmbedSequential(                
                            get_motion_module(
                            in_channels=in_channel_mm,
                            motion_module_type=self.motion_module_type, 
                            motion_module_kwargs=self.motion_module_kwargs,
                    )))
                ch = out_ch
        ...
        if self.use_motion_module:
            self.middle_block_motion_module = TimestepEmbedSequential(
                get_motion_module(
                    in_channels=ch,
                    motion_module_type=self.motion_module_type, 
                    motion_module_kwargs=self.motion_module_kwargs,
